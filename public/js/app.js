...
import { renderCampagne, initCampagne } from './render-campagne.js';
import { renderArchief, initArchief } from './render-archief.js';
import { renderDashboard, initDashboard } from './render-dashboard.js';
...
function updateFab() {
  const fab = $('#fab');
  if (state.role === 'dm' && state.activeSection !== 'dashboard') {
...
function onFabClick() {
  if (state.activeSection === 'campagne') {
    import('./render-campagne.js').then(m => m.openEditor());
  } else if (state.activeSection === 'archief') {
    import('./render-archief.js').then(m => m.openArchiefEditor());
  }
}
...
async function refreshSection(section) {
  ...
  if (section === 'campagne') await renderCampagne();
  else if (section === 'archief') await renderArchief();
  else if (section === 'dashboard') await renderDashboard();
}
